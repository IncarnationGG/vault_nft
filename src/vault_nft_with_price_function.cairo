...
pub mod VaultNFTWithPriceFunction {
    ...
    pub impl VaultNFTImpl of interfaces::IVaultNFT<ContractState> {
        ...
        fn convert_to_nft(self: @ContractState) -> u256 {
            self.price_function(self.erc721_enumerable.total_supply())
        }

        //burn after this
        fn convert_to_assets(self: @ContractState, token_id: u256) -> u256 {
            self.price_function(self.erc721_enumerable.total_supply() - 1)
        }
    ...
    pub impl InternalImpl of InternalTrait {
        fn price_function(self: @ContractState, index: u256) -> u256 { 
            self.base_price.read() + self.base_price.read() * index / 10000
        }
    }
}
